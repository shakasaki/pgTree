...
plt.figure()
for tree in trees:
    electrodes = pd.read_table('geometry_' + str(tree) + '.txt', header=None).loc[:, 1:2].to_numpy()
    tree_dict[tree] = {}
...
geom_16 = pd.read_table('16_electrode_geometry.txt', header=None).loc[:, 1:4].to_numpy()
geom_18 = pd.read_table('18_electrode_geometry.txt', header=None).loc[:, 1:4].to_numpy()
